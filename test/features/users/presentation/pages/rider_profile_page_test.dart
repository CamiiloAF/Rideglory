...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockRiderProfileCubit.stream,
    ).thenAnswer((_) => const Stream.empty());
    when(
      () => mockRiderProfileCubit.fetchRiderProfile(any()),
    ).thenAnswer((_) async {});
  ...
  group('RiderProfilePage — State Display Tests (US-2-3)', () {
    testWidgets('TC-2-26: Loading state shows loading widget', (
      WidgetTester tester,
    ) async {
      when(
        () => mockRiderProfileCubit.state,
      ).thenReturn(const ResultState.loading());
    ...
    testWidgets('TC-2-27: Data state shows rider name', (
      WidgetTester tester,
    ) async {
      when(
        () => mockRiderProfileCubit.state,
      ).thenReturn(const ResultState.data(data: mockUser));
    ...
    testWidgets('TC-2-30: Initial state renders without crash', (
      WidgetTester tester,
    ) async {
      when(
        () => mockRiderProfileCubit.state,
