...
## Phase 2 — Architect | model: opus (iter 1 or novel patterns) / sonnet (iter 2+ extending existing features)
<!-- Use opus when: iteration 1, new auth/WebSocket/cross-cutting patterns, or if iter 1 architect handoff was incomplete.
     Use sonnet when: extending an existing feature (vehicles, events, etc.) with established patterns. -->
...
## Phase 6 — QA | model: haiku

Spawn an Agent with:
- **description:** `"[QA / haiku] — iter $ARGUMENTS dart analyze + flutter test + sign-off"`
- **model:** `"haiku"`
- **prompt:**
...
CONTEXT — read in this order (minimum set; skip PRD and design.md to save tokens):
1. `docs/handoffs/architect-for-qa.md` — test commands + acceptance criteria. Read first.
2. `docs/handoffs/po.md` — stories and acceptance criteria
3. `docs/handoffs/frontend.md` — what was implemented, test commands
4. `docs/handoffs/backend.md` — rideglory-api changes (if exists)
5. `docs/handoffs/qa.md` (if exists — your prior test catalog)
6. `.claude/skills/qa-skill.md` (if exists)
7. `workflow/state.json` — partial read
...
## Phase 7 — DevOps | model: haiku

Spawn an Agent with:
- **description:** `"[DevOps / haiku] — iter $ARGUMENTS Flutter CI pipeline"`
- **model:** `"haiku"`
- **prompt:**
...
CONTEXT — read in this order (skip PRD — not needed for CI YAML):
1. `docs/handoffs/architect-for-devops.md` — CI changes, new env var names. Read first.
2. `docs/handoffs/qa.md` — test commands to wire into CI
3. `docs/handoffs/devops.md` (if exists — your prior CI config)
4. `.claude/skills/devops-skill.md` (if exists)
...
## Phase 10 — PO Close-out | model: haiku

Spawn an Agent with:
- **description:** `"[PO / haiku] — iter $ARGUMENTS close-out"`
- **model:** `"haiku"`
- **prompt:**
...
CONTEXT — read in this order (lean set for summary writing):
1. `docs/handoffs/po.md` — stories delivered, scope decisions
2. `docs/handoffs/qa.md` — test results and bugs
3. `docs/handoffs/tech_lead.md` — PR URL and review verdict
4. `docs/handoffs/frontend.md` — § Summary section only
5. `docs/handoffs/backend.md` — § Summary section only (if exists)
6. `workflow/state.json` — partial read: currentIteration, last 5 events, iteration row
7. `docs/PRODUCT_STATUS.md` (if exists)
8. `docs/ITERATION_HISTORY.md` (if exists)
9. `scripts/templates/iteration_checkpoint.md` (reset template)
