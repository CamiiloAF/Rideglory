...
`lib/` ya está reconstruido sobre Supabase: **Bloques 0, 1, 2 y 3 de `docs/product/ALCANCE-V2.md` están implementados** (cimientos, auth, perfil, garaje, documentos, mantenimiento, eventos con inscripción, y mapa en vivo, tracking y SOS). El **Bloque 3 se desbloqueó el 2026-09-10 por decisión del fundador**, sin esperar el experimento 1 de `docs/product/validacion.md` — esa validación sigue pendiente; las premisas de `ALCANCE-V2.md` §Bloque 3 se tratan como criterios de aceptación ya cerrados, no como hipótesis abiertas. Ver `docs/dev-runs/refactor-v2.md` para el cierre completo de esta corrida y su sección "Bloque 3 (2026-09-10)".
...
- **El SOS nunca falla en silencio.** Se persiste localmente antes de intentar la red, sobrevive a que maten la app, y toda ruta de fallo termina en alerta entregada o en fallback ofrecido. Nunca en un `return` mudo. *Implementado en `refactor/v2` (Bloque 3, 2026-09-10): `SosOutbox` (`lib/features/live_ride/data/services/shared_preferences_sos_outbox.dart`) encola el SOS en `SharedPreferences` antes de llamar a la red, la RPC `raise_sos` es idempotente por `client_id`, y `SosOutboxRetryCubit` reintenta al recuperar conectividad y al abrir la app — ver `docs/features/live_ride.md`. Historia: la v1 no cumplía esto — `tracking_ws_client.dart` publicaba el SOS con `_channel?.sink.add(...)` sobre un WebSocket y lo descartaba sin persistencia si el canal estaba caído.*
...
- **`flutter_map` no renderiza en golden tests.** Las pantallas con mapa se auditan por sus overlays aislados (banner de SOS, tarjetas de rider, controles); el mapa es un gap conocido y documentado. En código: usar `showMapTiles: false` (`LiveRideView`/`LiveRideContent`) para dejar el `TileLayer` fuera del árbol en tests y goldens. *Historia: Mapbox se evaluó para el Bloque 3 y se descartó por D13 (SDK frágil, token secreto en Gradle, no renderiza en goldens) a favor de `flutter_map` + tiles raster por URL.*
- **Pencil no guarda el `.pen` a disco por sí solo.** Tras diseñar una pantalla hay que guardar en la app antes de commitear — un cambio hecho por `pencil-designer` que no se guarda explícitamente se pierde.
- **Los AVD viven en un SSD externo.** Una sesión sin permiso de volúmenes removibles no puede arrancar el emulador — bloquea cualquier e2e/Patrol hasta que se conceda ese permiso o se corra desde una sesión que sí lo tenga.
